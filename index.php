...
function post()
{
    ...
    print_notify($request['method'], $request['url'], 502, '程序获取服务器失败: ' . var_export($errors, true));
}
...
function get() {
    ...
    if (@gzcompress('test') == false) {
        print_notify('GET', $_SERVER['SCRIPT_FILENAME'], 200, '错误: 需要 zlib 模块！');
        exit(-1);
    }
    
    if (!function_exists('curl_version') && !ini_get('allow_url_fopen')) {
        print_notify('GET', $_SERVER['SCRIPT_FILENAME'], 200, '错误: 需要curl 模块或 allow_url_fopen 模块！');
        exit(-1);
    }
...
<div style="background-color: #fff;background-color: #fff;color:#555;font:12px/1.5 '微软雅黑','Tahoma','Helvetica','Arial','sans-serif';width: 500px;margin:50px auto;border: 1px solid #ddd;border-top: none;-webkit-box-shadow: #ddd 0px 3px 7px;">
	...
	<tr valign="top" height="2">
		<td width="170" bgcolor="#00B1FF">
		</td>
		<td width="110" bgcolor="#66DBFF">
		</td>
		<td width="60" bgcolor="#96C3D2">
		</td>
		<td width="60" bgcolor="#88DDFC">
		</td>
